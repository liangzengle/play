    ...
    cache
      ...
      .addFunction(
        FunSpec.builder("remove")
          .addModifiers(KModifier.OVERRIDE)
          .addParameter("e", E_Entity_ID)
          .addStatement("removeById(e.%L)", getId())
          .build()
      )
      .addFunction(
        FunSpec.builder("removeById")
          .addModifiers(KModifier.OVERRIDE)
          .addParameter("id", idType)
          .addCode(
            CodeBlock.builder()
      ...
      .addFunction(
        FunSpec.builder("save")
          ...
          .addParameter("e", E_Entity_ID)
          .addStatement("requireNotDeleted(e.%L)", getId())
          .addStatement("val opt = getCached(e.%L)", getId())
          .beginControlFlow("if (opt.isPresent && opt.get() !== e)")
          .addStatement(
            """throw IllegalStateException("${'$'}{entityClass.simpleName}(${'$'}{e.%L})保存失败，与缓存中的对象不一致")""",
            getId()
          )
          .nextControlFlow("else")
          .addStatement("persistService.update(e)")
          .endControlFlow()
          .build()
      )
      .addFunction(
