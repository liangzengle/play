...
inline class Future<T>(private val cf: CompletableFuture<T>) {
  ...
  fun <U> map(f: (T) -> U): Future<U> {
    return Future(cf.thenApplyAsync(f))
  }

  fun <U> flatMap(f: (T) -> Future<U>): Future<U> {
    return Future(cf.thenComposeAsync { f(it).toCompletableFuture() })
  }
  ...
  fun andThen(f: (T, Throwable) -> Unit): Future<T> {
    return Future(cf.whenCompleteAsync { v, e -> f(v, e) })
  }
  ...
  fun onSuccess(f: (T) -> Unit): Future<T> {
    return Future(cf.whenCompleteAsync { v, e -> if (e == null) f(v) })
  }

  fun onFailure(f: (Throwable) -> Unit): Future<T> {
    return Future(cf.whenCompleteAsync { _, e -> if (e != null) f(e.cause ?: e) })
  }
