...
public class CronSequenceGenerator {
    ...
    private String replaceOrdinals(String value, String commaSeparatedList) {
        ...
        for (int i = 0; i < list.length; i++) {
            ...
            value = value.toUpperCase().replace(item, String.valueOf(i));
        }
        return value;
    }
    ...
    private static String[] tokenizeToStringArray(String expression, String delim) {
        if (expression == null) {
            return null;
        }
    ...
    public Date prev(Date date) {
        ...
        do {
            ...
            prevFireTime = next(prevCheckDate);
            if (prevFireTime == null || prevFireTime.before(MIN_DATE)) {
                return null;
            }
            start = prevCheckDate;
        } while (prevFireTime.compareTo(date) >= 0);
        return prevFireTime;
    }

    private long findMinIncrement() {
        int secondsCardinality = seconds.cardinality();
    ...
    private int minInSet(BitSet set) {
        int previous = 0;
        int min = Integer.MAX_VALUE;
        boolean first = true;
        PrimitiveIterator.OfInt it = set.stream().iterator();
        while (it.hasNext()) {
