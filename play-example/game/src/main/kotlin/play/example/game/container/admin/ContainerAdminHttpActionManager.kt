...
class ContainerAdminHttpActionManager : HttpActionManager() {
  private var actionManagers = IntObjectMaps.immutable.empty<HttpActionManager>()

  @Synchronized
  fun registerActionManager(serverId: Int, manager: HttpActionManager) {
    actionManagers = actionManagers.newWithKeyValue(serverId, manager)
  }
  ...
  fun findAction(serverId: Int, path: String, uri: String): Optional<Action> {
    val actionManager = actionManagers.get(serverId) ?: return Optional.empty()
