...
import play.example.game.module.reward.RewardService
import play.example.game.module.reward.model.Reward
import play.example.game.module.reward.model.RewardResultSet
...
import play.example.game.module.task.entity.TaskStatus
...
abstract class AbstractTaskService<PlayerTask : AbstractTask, TaskConfig : AbstractTaskConfig> {
  ...
  protected fun checkIfFinished(task: PlayerTask, taskConf: TaskConfig): Boolean {
    if (taskConf.targets.size > task.progressesSize()) {
      return false
    }
    for (i in taskConf.targets.indices) {
