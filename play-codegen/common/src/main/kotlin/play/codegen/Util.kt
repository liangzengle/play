...
fun isObjectMethod(elem: ExecutableElement) = objectMethodNames.contains(elem.simpleName.toString())

fun resolveTypeVariables(type: TypeName, typeTable: Map<String, TypeName>): TypeName {
...
fun replaceType(type: TypeName, target: TypeName, replacement: TypeName): TypeName {
  if (type == target) {
    return replacement
  }
  if (type is ParameterizedTypeName) {
...
fun String.uncapitalize() = this.replaceFirstChar { it.lowercaseChar() }

fun toParamStr(parameters: List<VariableElement>): String {
  return parameters.joinToString(separator = ", ") { it.simpleName }
}

inline fun <reified T> qualifiedName() = T::class.qualifiedName!!

fun String.capitalize2() = replaceFirstChar { it.uppercaseChar() }
fun String.uncapitalize2() = replaceFirstChar { it.lowercaseChar() }
